...
onMounted(() => {
  const antibiotics = [
    ...
    ["penicillin", "streptomycin", "neomycin"].map((drug) => ({
      drug,
      mic: d[drug],
      bacteria: d.bacteria,
      gram: d.gram,
  ...
  const min = Math.sqrt(Math.log(0.001 * 1e4));
  ...
  const svg = d3
    ...
    .attr(
      "style",
      "max-width: 100%; height: auto; background: rgb(240, 225, 210);"
    );
  ...
  svg
    ...
    .attr("fill", (d) =>
      gramColor(antibiotics.find((b) => b.bacteria === d).gram)
    );
  ...
  svg
    ...
    .attr(
      ...
      d3
        ...
        .endAngle(
          (d) =>
            nameAngle(d.bacteria) + drugAngle(d.drug) + drugAngle.bandwidth()
        )
    )
  ...
  svg
    ...
    .call((g) =>
      g.append("circle").attr("cx", -12).attr("r", 6).attr("fill", gramColor)
    )
    .call((g) =>
      g
        .append("text")
